 ...
 */
function {PROFILE_CODE}_image_default_styles() {
  $styles = array();
 ...
 */
function {PROFILE_CODE}_form_install_configure_form_alter(&$form, $form_state) {
  ...
  $form['admin_account']['account']['{PROFILE_CODE}_name'] = array(
    '#type' => 'item',
    '#title' => st('Username'),
    '#markup' => 'admin'
  );
  $form['admin_account']['account']['{PROFILE_CODE}_password'] = array(
    '#type' => 'item',
    '#title' => st('Password'),
    '#markup' => 'admin'
  );
  $form['admin_account']['account']['{PROFILE_CODE}_informations'] = array(
  ...
  array_unshift($form['#validate'], '{PROFILE_CODE}_custom_setting');
}
 ...
 * custom values.
 */
function {PROFILE_CODE}_custom_setting(&$form, &$form_state) {
 ...
 */
function {PROFILE_CODE}_system_info_alter(&$info, $file, $type) {
...
function {PROFILE_CODE}_update_projects_alter(&$projects) {
  ...
  _update_process_info_list($projects, array('{PROFILE_CODE}' => $kickstart), 'module', TRUE);
}
 ...
 */
function {PROFILE_CODE}_update_status_alter(&$projects) {
  $bad_statuses = array(
    UPDATE_NOT_SECURE,
    UPDATE_REVOKED,
  ...
  $make_filepath = drupal_get_path('module', '{PROFILE_CODE}') . '/drupal-org.make';
  ...
  foreach ($projects as $project_name => $project_info) {
    ...
    if (strpos($project_name, '{PROFILE_CODE}_') !== FALSE) {
...
function {PROFILE_CODE}_form_update_manager_update_form_alter(&$form, &$form_state, $form_id) {
  if (isset($form['projects']['#options']) && isset($form['projects']['#options']['{PROFILE_CODE}'])) {
    if (count($form['projects']['#options']) > 1) {
      unset($form['projects']['#options']['{PROFILE_CODE}']);
    }
    else {
      unset($form['projects']);
 ...
 */
function {PROFILE_CODE}_crumbs_get_info() {
  $crumbs = array(
    'crumbs.home_title' => 0
  );

  foreach (module_implements('{PROFILE_CODE}_crumb_info') as $module) {
    // The module-provided item might be just the name of the plugin, or it
    // might be an array in the form of $plugin_name => $weight.
    foreach (module_invoke($module, '{PROFILE_CODE}_crumb_info') as $crumb) {
      if (is_array($crumb)) {
        $crumbs += $crumb;
      }
      else {
